...
#2) The actual plotting function; 
     ...
     mirror_plot_func <- function(d1=d1, new_data=new_data, chr = NULL, y_min = NULL, y_max = NULL, y_ticks = NULL, zoom_left = NULL, zoom_right = NULL, gene_tag_p = NULL, x=df_for_ticks, chr_mid_points = "chr_mid_points", chrs = "chrs", store_last_positions = "store_last_positions", color_tissue = NULL, sig_line1 = NULL, sig_line2 = NULL, sig_line1_color = NULL, sig_line2_color = NULL) {
	...
	##Modifying gene-specific dataset for plotting
	   d1$new_pos <- NA
       d1$col_cat2 <- NA
       for (i in order(unique(d1$CHR))) {
         if(i == df_for_ticks$chrs[1]) {
           d1$new_pos[which(d1$CHR == i)] <- d1$MID_POS[which(d1$CHR == i)]
         }
         else {
           d1$new_pos[which(d1$CHR == i)] <- d1$MID_POS[which(d1$CHR == i)] + df_for_ticks$store_last_positions[which(df_for_ticks$chrs == i)-1]
         }
       }
       ...
       d1$col_cat2 <- NA
       for (i in 1:length(order(unique(d1$CHR)))) {
         d1$col_cat[which(d1$CHR == order(unique(d1$CHR))[i])] <- as.numeric(i)
         d1$col_cat2[which(d1$col_cat == i)] <- as.numeric(i) %% 2
       }
  ...
  ##Color Tissue Null option      
	...
	color_tissue =  c("bisque3","peachpuff2","lightgreen","firebrick4","red","orangered", "powderblue", "blue4","darkturquoise","dodgerblue","cyan","steelblue", 
      ...
      if((is.null(chr) & is.null(zoom_left) & is.null(zoom_right)) == TRUE) {
		...
		#Setting default y-minima and y-maxima for plots; 
		if(is.null(y_min) == TRUE) {y_min <- round(min(new_data$log_p, na.rm=TRUE) - 1)} else {y_min <- y_min}
		if(is.null(y_max) == TRUE) {y_max <- round(max(d1$log_p, na.rm=TRUE) + 1)} else {y_max <- y_max}
		if(is.null(y_ticks) == TRUE) {
			if((y_max - y_min) > 16) {y_ticks <- 16} else {y_ticks <- round(y_max - y_min)} 
		} else {y_ticks <- y_ticks}
    ...
    if(is.null(sig_line2_color) == TRUE) {sig_line2_color <- "black"} else {sig_line2_color <- sig_line2_color}
       ...
       theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank()) +
        ...
        geom_point(data = d1, aes(x = new_pos, y = log_p, size = 1.0, color = dtype, shape = factor(col_cat2))) +
        xlab("Chromosomes") + ylab(test_ylab) +
       scale_y_continuous(breaks=seq(y_min, y_max, break_length)) + expand_limits(y=c(y_min, y_max)) +
       geom_label_repel(data = for_tag_d1, aes(x = new_pos, y = log_p, label = Gene)) +
    
        geom_point(data = new_data, aes(x = new_pos, y = -log_p, size = 1.0, color = dtype, shape = factor(col_cat2))) +
         scale_colour_manual(name = "Tissue Type", values = color_tissue, labels = labels_cat) +
        ...
        geom_label_repel(data = for_tag_new_data, aes(x = new_pos, y = -log_p, label = Gene)) +
   ...
   if(is.null(sig_line2) == TRUE) {sig_line2 <- 0} else{sig_line2 <- sig_line2}
    ...
    if(is.null(sig_line2_color) == TRUE) {sig_line2_color <- "black"} else {sig_line2_color <- sig_line2_color}
        ...
        p1 <- ggplot() +  theme_bw() +  
          ...
          geom_point(data = subset(d1, CHR == chr), aes(x = MID_POS, y = log_p, size = 1.0, color = dtype, shape = factor(col_cat2))) +
	  ...
	  scale_y_continuous(breaks=seq(y_min, y_max, break_length)) + expand_limits(y=c(y_min, y_max)) +
          ...
          #scale_colour_manual(values = c("darkgray", "black", color_tissue)) +
          scale_colour_manual(name = "Tissue Type", values = c(color_tissue), labels = c(labels_cat)) +
          #scale_colour_manual(values = c("darkgray", "black", color_tissue)) +
          #guides(shape = "none", size = "none", col = guide_legend(reverse = TRUE), colour = guide_legend(reverse = TRUE, override.aes = list(size=10))) + 
          guides(shape = "none", size = "none", colour = guide_legend(reverse = TRUE, override.aes = list(size=6))) + 
         
	  #geom_label_repel(data = subset(new_data, CHR == chr & log_p > gene_tag_p), aes(x = MID_POS, y = log_p, label = Gene)) +
	      geom_label_repel(data = for_tag_d1, aes(x = MID_POS, y = log_p, label = Gene)) +
          geom_label_repel(data = for_tag_new_data, aes(x = MID_POS, y = log_p, label = Gene)) +
	...
	#geom_hline(aes(yintercept = sig_line1), color = "red", size = 1) +
 	 ...
 	 new_data3$LINE_POS <- new_data3$LINE_POS/1000000
          ...
          d11 <- data.frame(CHR=d1$CHR, Gene=d1$Gene, LINE_POS=d1$START_POS, log_p=d1$log_p, dtype=d1$dtype, tag_identifier=0)
          d12 <- data.frame(CHR=d1$CHR, Gene=d1$Gene, LINE_POS=d1$END_POS, log_p=d1$log_p, dtype=d1$dtype, tag_identifier=1)
          d13 <- rbind(d11, d12)
          d13$Gene_dtype <- paste(d13$Gene, d13$dtype, sep="_")
 	 #d13$ENSG_dtype <- paste(d13$ENSG, d13$dtype, sep="_")
 	 d13$LINE_POS <- d13$LINE_POS/1000000       
     ...
     if(is.null(sig_line2_color) == TRUE) {sig_line2_color <- "black"} else {sig_line2_color <- sig_line2_color}
          ...
          p1 <- ggplot() +  theme_bw() +  
            ...
            geom_point(data = subset(d1, CHR == chr & LINE_POS >= zoom_left & LINE_POS <= zoom_right), aes(x=LINE_POS, y = log_p, group = Gene_dtype, size = 1.5, color = dtype)) +
            xlab(paste("Chromosome", chr, "(Mb)", sep = " ")) + ylab(test_ylab) +
            scale_y_continuous(breaks=seq(y_min, y_max, break_length)) + expand_limits(y=c(y_min, y_max)) +
