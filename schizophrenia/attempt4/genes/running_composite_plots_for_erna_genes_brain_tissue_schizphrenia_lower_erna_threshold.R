source(file="/home/bettimj/aldrich_rotation/metaxcan/miami_metaxcan_plot_functions_gtexv8.R")
library(data.table)

#############Loading GWAS file first: 

gwas_summary <- fread("/home/bettimj/gamazon_rotation/eRNA_predixcan_models/attempt4/schizophrenia/PGC3_SCZ_wave3.primary.autosome.public.v3.vcf.tsv", header= TRUE, sep = "auto") 
#########Change name of GWAS file here; If needed change seperator as needed, currently it is set to whitespace(space and/or tab)
...
metaxcan_results_files_gene <- list.files(path = "/home/bettimj/gamazon_rotation/eRNA_predixcan_models/attempt4/schizophrenia/twas_all_genes/brain_only", pattern = "*.csv$")  ####################May want to change here, or move desired files to a new directory
metaxcan_results_files_erna <- list.files(path = "/home/bettimj/gamazon_rotation/eRNA_predixcan_models/attempt4/schizophrenia/twas_all/brain_only", pattern = "*.csv$")

##Create a vector with elements in the same order as your *.csv files 
c1 <- c("Brain Amygdala", "Brain Anterior cingulate cortex BA24", "Brain Caudate basal ganglia", "Brain Cerebellar Hemisphere", "Brain Cerebellum", "Brain Cortex", "Brain Frontal Cortex_BA9", "Brain Hippocampus", "Brain Hypothalamus", "Brain Nucleus accumbens basal ganglia", "Brain Putamen basal ganglia", "Brain Spinal cord cervical c-1", "Brain Substantia nigra") #############Check to make sure the order matches metaxcan_results_files; or do ls *.csv to check order #############Check to make sure the order matches metaxcan_results_files; or do ls *.csv to check order
...
for (i in 1:length(track_names_gene$name_list)) {
  ...
  if(!exists("appended_gene_d_gene")){
    a_gene <- fread(paste0("/home/bettimj/gamazon_rotation/eRNA_predixcan_models/attempt4/schizophrenia/twas_all_genes/brain_only/", as.character(track_names_gene$file_name[i])), header = T, sep="auto")
...
###	}
    ...
    a_gene <- read.csv(file = paste0("/home/bettimj/gamazon_rotation/eRNA_predixcan_models/attempt4/schizophrenia/twas_all_genes/brain_only/", as.character(track_names_gene$file_name[i])), header = T)
...
for (i in 1:length(track_names_erna$name_list)) {
  ...
  if(!exists("appended_gene_d_erna")){
    a_erna <- fread(paste0("/home/bettimj/gamazon_rotation/eRNA_predixcan_models/attempt4/schizophrenia/twas_all/brain_only/", as.character(track_names_erna$file_name[i])), header = T, sep="auto")
...
###	}
    ...
    a_erna <- read.csv(file = paste0("/home/bettimj/gamazon_rotation/eRNA_predixcan_models/attempt4/schizophrenia/twas_all/brain_only/", as.character(track_names_erna$file_name[i])), header = T)
...
tiff(filename = "miami_Entire_Genome_result_schizophrenia_erna_gene_brain_tissue_lower_erna_threshold.tiff", height= 10, width = 20, unit = "in", res=300, pointsize=11, compression ='lzw') ###Optional, filename change for output if desired. 
...
x=df_for_ticks, chr_mid_points = "chr_mid_points", chrs = "chrs", color_tissue = c("powderblue", "blue4","darkturquoise","dodgerblue","cyan","steelblue", 
			"lightslateblue","skyblue1", "royalblue","slateblue3","deepskyblue1", "turquoise", "midnightblue"), 
y_min = NULL, y_max = NULL, y_ticks = NULL, sig_line1 = 5.157, sig_line2 = -6.838, sig_line1_color =  c("darkred"), sig_line2_color = c("darkred"))
dev.off()
...
fname_print <- paste("Chr_", df_for_ticks$chrs[i], "miami_Entire_Genome_result_schizophrenia_erna_gene_brain_tissue_lower_erna_threshold.tiff", sep = "") ###Modify name of file as you wish
tiff(filename = fname_print, height = 10, width = 20, unit ="in", res =300, pointsize =11, compression = 'lzw')
mirror_plot_func(d1=appended_gene_d_erna, new_data=appended_gene_d_gene, chr = df_for_ticks$chrs[i], zoom_left = NULL,  zoom_right = NULL, gene_tag_p = 3.0, 
x=df_for_ticks, chr_mid_points = "chr_mid_points", chrs = "chrs", color_tissue = c("powderblue", "blue4","darkturquoise","dodgerblue","cyan","steelblue", 
			"lightslateblue","skyblue1", "royalblue","slateblue3","deepskyblue1", "turquoise", "midnightblue"), 
y_min = NULL, y_max = NULL, y_ticks = NULL, sig_line1 = 5.157, sig_line2 = -6.838, sig_line1_color =  c("darkred"), sig_line2_color = c("darkred"))
dev.off()
...
d1 <- appended_gene_d_erna
new_data <- appended_gene_d_gene

for_tag_d1 <- subset(d1, log_p > gene_tag_p)
for_tag_d1 <- for_tag_d1[order(for_tag_d1$Gene, -for_tag_d1$log_p),]
for_tag_d1 <- for_tag_d1[!duplicated(for_tag_d1$Gene),]
    
for_tag_new_data <- subset(new_data, log_p > gene_tag_p)
for_tag_new_data <- for_tag_new_data[order(for_tag_new_data$Gene, -for_tag_new_data$log_p),]
for_tag_new_data <- for_tag_new_data[!duplicated(for_tag_new_data$Gene),]

gene_tag_p <- 3.0

fname_print <- paste("Chr_", "1", "miami_Entire_Genome_result_schizophrenia_erna_gene_brain_tissue_lower_erna_threshold.tiff", sep = "") ###Modify name of file as you wish
tiff(filename = fname_print, height = 10, width = 20, unit ="in", res =300, pointsize =11, compression = 'lzw')
mirror_plot_func(d1=appended_gene_d_erna, new_data=appended_gene_d_gene, chr = 1, zoom_left = NULL,  zoom_right = NULL, gene_tag_p = 3.0, 
x=df_for_ticks, chr_mid_points = "chr_mid_points", chrs = "chrs", color_tissue = c("powderblue", "blue4","darkturquoise","dodgerblue","cyan","steelblue", 
			"lightslateblue","skyblue1", "royalblue","slateblue3","deepskyblue1", "turquoise", "midnightblue"), 
y_min = NULL, y_max = NULL, y_ticks = NULL, sig_line1 = 5.157, sig_line2 = -6.838, sig_line1_color =  c("darkred"), sig_line2_color = c("darkred"))
dev.off()
