...
gwas_summary <- fread("/home/bettimj/gamazon_rotation/eRNA_predixcan_models/gwas_sum_stats/lung_cancer/for_metaxcan_in/Overall.txt", header= TRUE, sep = "auto")  #########Change name of GWAS file here; If needed change seperator as needed, currently it is set to whitespace(space and/or tab)

gwas_df <- new_xaxis_func_v2(gwas_summary, chr_name = "CHR", pos_name = "position", p_val_name = "Pvalue_random", snp_name = "alternate_ids") ##########Specify column names here
...
tiff(filename = "miami_Entire_Genome_result_schizophrenia_erna_gene_brain_tissue.tiff", height= 10, width = 20, unit = "in", res=300, pointsize=11, compression ='lzw') ###Optional, filename change for output if desired. 
...
x=df_for_ticks, chr_mid_points = "chr_mid_points", chrs = "chrs", color_tissue = c("powderblue", "blue4","darkturquoise","dodgerblue","cyan","steelblue", 
			"lightslateblue","skyblue1", "royalblue","slateblue3","deepskyblue1", "turquoise", "midnightblue"), 
y_min = NULL, y_max = NULL, y_ticks = NULL, sig_line1 = 5.911, sig_line2 = -6.838, sig_line1_color =  c("darkred"), sig_line2_color = c("darkred"))
dev.off()
