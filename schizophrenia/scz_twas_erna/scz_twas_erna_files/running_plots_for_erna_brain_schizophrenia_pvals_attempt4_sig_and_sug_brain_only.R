source(file="/home/bettimj/aldrich_rotation/metaxcan/metaxcan_plot_functions_gtexv8_sig_and_sug.R")
library(data.table)
...
metaxcan_results_files <- list.files(pattern = "schizophrenia2022_eRNA_imputations.Brain*")  ####################May want to change here, or move desired files to a new directory
...
tiff(filename = "Entire_Genome_labeled_result_schizophrenia_erna_brain_tissues_att4.tiff", height= 10, width = 20, unit = "in", res=300, pointsize=11, compression ='lzw') ###Optional, filename change for output if desired. 
mirror_plot_func(d1=gwas_df_d1, new_data=appended_gene_d, chr = NULL, zoom_left = NULL,  zoom_right = NULL, gene_tag_p = 3.0, 
x=df_for_ticks, chr_mid_points = "chr_mid_points", chrs = "chrs", color_tissue = c("powderblue", "blue4","darkturquoise","dodgerblue","cyan","steelblue", 
			"lightslateblue","skyblue1", "royalblue","slateblue3","deepskyblue1", "turquoise", "midnightblue"), 
y_min = NULL, y_max = NULL, y_ticks = NULL, sig_line1 = 5.911, sig_line1_sug = 5.461, sig_line2 = -7.301, sig_line2_sug = -6, sig_line1_color =  c("darkred"), sig_line1_sug_color =  c("blue"), sig_line2_color = c("darkred"), sig_line2_sug_color = c("blue"))
dev.off()
