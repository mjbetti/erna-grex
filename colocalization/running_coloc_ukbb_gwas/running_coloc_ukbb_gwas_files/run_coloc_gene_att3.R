...
out_dir <- "/home/bettimj/gamazon_rotation/eRNA_predixcan_models/attempt4/gwas_snp_coloc/att3/gene"
...
gene_path <- "/home/bettimj/gamazon_rotation/eRNA_predixcan_models/attempt4/gwas_snp_coloc/gene_all_tissues_eqtls_matrix_eqtl.with_coord.arm.filt_fdr.txt"
gene_file <- fread(gene_path, header = TRUE, sep = "\t", quote = "")
gene_df <- as.data.frame(gene_file)
names(gene_df)[1] <- "varid"
gene_df <- gene_df[(gene_df$tissue == tissue),]

maf_gene_path <- "/home/bettimj/gamazon_rotation/gtex8_geno/liftover_hg19/plink2.afreq"
maf_gene_file <- fread(maf_gene_path, header = TRUE, sep = "\t", quote = "")
maf_gene_df <- as.data.frame(maf_gene_file)

gene_df <- merge(gene_df, maf_gene_df, by.x = "varid", by.y = "ID")
...
for (gwas_file_name in list.files(in_dir, pattern = "*.txt")) {
  ...
  eqtl_locus <- gene_df[(gene_df$arm_vector == locus),]
...
out_file <- paste(tissue, "coloc_ukbb_sig.gene.txt", sep = "-")
