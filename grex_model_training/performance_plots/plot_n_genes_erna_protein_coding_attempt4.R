...
for (tissue in tissues) {
	...
	n_genes <- length(list.files(weights_dir))
	new_row <- c(tissue, n_genes)
...
names(n_erna_df) <- c("tissue", "genes", "Category")
names(n_genes_df) <- c("tissue", "genes", "Category")

#Combine together
merged_df <- rbind(n_erna_df, n_genes_df)

pdf("erna_protein_coding_n_genes_attempt4.pdf")
ggplot(merged_df, aes(factor(tissue), as.numeric(genes), fill = Category)) + 
  geom_bar(stat="identity", position = "dodge") + 
  coord_flip() +
  scale_y_continuous(name = "Genes", breaks = c(0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000, 13000), 
                     labels = c(0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000, 11000, 12000, 13000), 
                     limit = c(0, 13000),
                     expand = c(0, 0)) +
